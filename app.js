...
function runScript(script, eventType, event, argument="") {
	...
	eventStr = eventStr.replace(/\\/gi, '\\\\');
	eventStr = eventStr.replace(/"/gi, '\\\"');
	eventStr = eventStr.replace(/\$/gi, '\\$');


	const child = exec(`${script} "${eventType}" "${eventStr}"  ${argument}`  , function (error, stdout, stderr) {
...
function findToUpdate(res, id, ref, name, eventType, event, argument) {
	...
	if (!foundServer) {
		...
		res.json({
			"status": 404,
			"message": "Rule for \"" + (eventType === undefined ? "none" : eventType)  + "\" on repository \"" + name + "\" (" + id + ") not found"
		});
	}
}
